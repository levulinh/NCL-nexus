...
COPY . /nexus
RUN apt update \
 && apt install -y unzip build-essential git autoconf automake libtool pkg-config curl make zlib1g-dev wget \
...
RUN apt update \
 && apt install -y libswscale4 libjpeg8 libpng16-16 \
 ...
 && apt purge -y python3.7-dev software-properties-common wget \
 && apt autoremove -y \
